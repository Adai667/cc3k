...
DlcGamesys::DlcGamesys():end{false},re{false}{}
void DlcGamesys::startfloors(string floorfile,string chamberfile)
{
...
DlcFloor& DlcGamesys::getfloor1()
{
   return floor1;
}
DlcFloor& DlcGamesys::getfloor2()
{
   return floor2;
}
DlcFloor& DlcGamesys::getfloor3()
{
   return floor3;
}
DlcFloor& DlcGamesys::getfloor4()
{
   return floor4;
}
DlcFloor& DlcGamesys::getfloor5()
{
   return floor5;
}

bool DlcGamesys::endgame()
{
   return end;
}
void DlcGamesys::score()
{
   if(end || re) return;
}
bool DlcGamesys::replay()
{
   return re;
}
void DlcGamesys::playgame(DlcFloor &floor,int level, bool init)
{
   // init player
   ...
   while(isdone != true)
   {
	 ...
	 cin >> again;
      ...
      cout<<"Def: "<<def<<endl;
      cout<<"Action:";
      vector<Enemy*> targets = floor.enemyAround(playerx, playery, player->getChambernumber());
...
void DlcGamesys::initposition(Character *subject, char symbol,DlcFloor &floor)
{
   int subposx,subposy;
   random_device random_device;
   mt19937 engine{random_device()};
...
void DlcGamesys::initstair(Player *player,char symbol, DlcFloor &floor)
{
   while(true)
   {
     random_device random_device;
...
void DlcGamesys::initenemy(DlcFloor &floor)
{
